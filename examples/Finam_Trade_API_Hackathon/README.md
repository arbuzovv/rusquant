...
Для создания робота на Yandex Cloud необходимо создать клауд функция в R и поместить туда содержимое папки yandexcloudbot.

Файл index.R представляет собой сам код торгового робота

Файл packages.R представляет собой для установки библиотеки rusquant

Таймаут рекомендуется установить 60 секунд, а память до 1024Мбю
...
Но иногда интереснее и полезнее взглянуть на те торговые метрики, которые необходимы именно тебе.
Для построения фронтэнда использовался Yandex DataLens который позволяет в удобной форме визуализировать необходимые вам показатели.
[Ссылка](https://datalens.yandex.ru/4l4yydg8alqou-rusquant-bot) на фронтэнд торгового робота разработанного в рамках Хакатона.

![Alt text](img/frontend.png "Фронтэнд")
